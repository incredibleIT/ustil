...
/**
 * HomeView - 首页（神经网络背景版）
 * 基于 HomeViewButtonPlacementPOC.vue 完全复制，仅修改：
...
const heroButtonsRef = ref<HTMLDivElement | null>(null) // 保留引用（虽然不再使用）
...
const actionButtonsRef = ref<HTMLElement | null>(null) // 新增：功能按钮区域引用
...
onMounted(() => {
  ...
  console.log('📊 Canvas ref exists:', !!neuralCanvasRef.value)
  
  if (!neuralCanvasRef.value) {
    console.error('❌ Canvas ref is null! Page will be blank.')
    return
  }
  
  // 确保Canvas可见
  neuralCanvasRef.value.style.display = 'block'
  console.log('✅ Canvas display set to block')
  console.log('📐 Window size:', window.innerWidth, 'x', window.innerHeight)
  console.log('📐 Canvas element:', {
    offsetWidth: neuralCanvasRef.value.offsetWidth,
    offsetHeight: neuralCanvasRef.value.offsetHeight,
    clientWidth: neuralCanvasRef.value.clientWidth,
    clientHeight: neuralCanvasRef.value.clientHeight
  })
...
onUnmounted(() => {
  ...
  try {
    if (neuralCanvasRef.value) {
      ...
      if (ctx) {
        ctx.clearRect(0, 0, neuralCanvasRef.value.width, neuralCanvasRef.value.height)
      }
    }
  } catch (error) {
...
function initNeuralNetwork() {
  ...
  propagationIntervalId = setInterval(generatePropagation, 1500) as unknown as number
...
function setupScrollAnimations() {
  ...
  if (actionButtonsRef.value) {
    ...
    // 标题动画
    const titleEl = actionButtonsRef.value.querySelector('h2')
    if (titleEl) {
      gsap.fromTo(titleEl,
        { y: 40, opacity: 0 },
        {
    ...
    console.log('✅ Action buttons scroll animations configured')
  }
...
function setupLenisScrollTrigger() {
  ...
  // Lenis 的 requestAnimationFrame 循环（关键：让鼠标滚轮生效）
  function raf(time: number) {
    lenis.raf(time)
    lenisRafId = requestAnimationFrame(raf) as unknown as number
  }
  lenisRafId = requestAnimationFrame(raf) as unknown as number
}
</script>
...
<template>
  <div class="home-view-wrapper">
    ...
    <!-- 主容器 -->
    <div class="home-jiejoe min-h-screen bg-gradient-to-b from-white to-blue-50/30">
    ...
    <section ref="actionButtonsRef" class="relative z-10 py-24 bg-gradient-to-b from-white/80 to-indigo-50/50">
      <div class="max-w-6xl mx-auto px-4">
        ...
        <!-- 未登录状态 -->
        <div v-if="!authStore.isAuthenticated" class="flex flex-wrap gap-6 justify-center items-center">
          <AppButton
            variant="primary"
            size="lg"
    ...
    <!-- 核心功能展示区 -->
    <section ref="featuresSectionRef" class="relative z-10 py-32 bg-gradient-to-b from-indigo-50/50 to-blue-50/50">
